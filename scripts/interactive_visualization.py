...
def main(projection, path_im, path_size=None, selection=None, palette="magma_r"):
    ...
    image_path = np.asarray(
        [
            glob("{}/tile_{}.jpg".format(path_im, o))[0]
            for o in range(projection.shape[0])
        ]
    )
    ...
    global axes_im
    axes_im = [
        plt.subplot2grid(gridsize, (o, 2), rowspan=1, colspan=1, fig=fig)
        for o in range(2)
    ]
    [
        (
            x.set_xticks([]),
            x.set_yticks([]),
            x.set_navigate(False),
            x.imshow(plt.imread(image_path[0])),
        )
        for x in axes_im
    ]
    line = sns.scatterplot(
        x=projection[:, 0],
        y=projection[:, 1],
        size=50,
        alpha=0.7,
        ...
        if line.contains(event)[0]:
            ...
            axes_im = [
                (
                    set_axes_color(ax, "r")
                    if o == counter % 2
                    else set_axes_color(ax, "black")
                )
                for o, ax in enumerate(axes_im)
            ]
...
parser.add_argument(
    "--target_val",
    type=str,
    help="See tiles corresponding to a particular value of the target variable. Coincide with the names of the folders in path/hightiles/",
)
parser.add_argument(
    "--decision_emb",
    action="store_true",
    help="when tagged, use the decision embeddings (related to the target variable).",
)
parser.add_argument(
    "--cluster",
    action="store_true",
    help="If tagged, tiles will be clustered. If not, color of the points corresponds to the decision score(tiles logits)",
)
...
if args.decision_emb:
    emb_path = os.path.join(
        args.path, "hightiles_encoded", f"{args.target_val}_preclassif.npy"
    )
else:
...
scores = np.load(
    os.path.join(args.path, "hightiles_encoded", f"{args.target_val}_scores.npy")
)
colors, palette = (labels, "bright") if args.cluster else (scores, "magma_r")
main(
    M,
    os.path.join(args.path, f"hightiles/{args.target_val}"),
    path_size=colors,
    palette="bright",
)
