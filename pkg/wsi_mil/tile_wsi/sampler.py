...
class TileSampler:
    def __init__(self, args, wsi_path, info_folder):
        ...
        if args.sampler in ["predmap", "predmap_all"]:
            ...
            path_predmap = (
                os.path.join(info_folder, name_wsi + "_predmap.npy")
                if os.path.exists(os.path.join(info_folder, name_wsi + "_predmap.npy"))
                else None
            )
    ...
    def predmap_all_sampler(self, nb_tiles):
        ...
        fpred1 = np.where(
            np.logical_and(self.predmap > -1, self.predmap <= 0.5).flatten()
        )[0]
        infomat_coord = [np.unravel_index(x, self.infomat.shape) for x in fpred1]
        if len(fpred1) < nb_tiles:
            infomat_coord += [
                np.unravel_index(x, self.infomat.shape)
                for x in np.random.permutation(fpred0)
            ][: nb_tiles - len(fpred1)]
        indices = np.array([int(self.infomat[x[0], x[1]]) for x in infomat_coord])
        return indices
