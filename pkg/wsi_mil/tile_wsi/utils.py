...
def get_polygon(image, path_xml):
    ...
    nrows = (
        doc.getElementsByTagName("imagesize")[0]
        .getElementsByTagName("nrows")[0]
        .firstChild.data
    )
    ncols = (
        doc.getElementsByTagName("imagesize")[0]
        .getElementsByTagName("ncols")[0]
        .firstChild.data
    )
...
def visualise_cut(
    ...
    for para in list_pos:
        ...
        patch = patches.Rectangle(
            plot_seed, width, height, fill=False, edgecolor=plot_args["color"]
        )
...
def make_auto_mask(slide, mask_level):
    ...
    mask = opening(
        closing(
            np.logical_and(im_gray < t, im_gray > 0.1).reshape(size),
            footprint=square(2),
        ),
        footprint=square(2),
    )
